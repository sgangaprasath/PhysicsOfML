...
\end{groupplot}
\end{tikzpicture}
\caption{Level sets (constant-density contours) of 2-D Gaussians. Left: isotropic ($\mathbf{C}=\mathbf{I}$), circular contours. Centre: positive correlation $\rho=0.7$, ellipse tilted at $45^\circ$. Right: negative correlation $\rho=-0.7$, ellipse tilted at $-45^\circ$. Dashed red lines mark the principal axes (eigenvectors of $\mathbf{C}$).}
...
\begin{scope}
\node[font=\small,anchor=north] at (1.3,-3) {Before HS};
...
\node[font=\small,anchor=north] at (1.3,-3) {After HS};
